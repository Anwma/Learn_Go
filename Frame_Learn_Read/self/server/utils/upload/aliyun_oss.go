...
//阿里云 作业支援系统operations support system
...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	if openError != nil {
		...
		return "", "", errors.New("function file.Open() Faile,err :" + openError.Error())
	}
	//创建文件及时使用defer关闭
	defer f.Close()
...
func (*AliyunOSS) DeleteFile(key string) error {
	...
	//删除单个文件。objectName 表示删除OSS文件时需要只当包含文件后缀在内的完整路径，例如abc/efg/123.jpg
	//如需删除文件夹，请将objectName设置为对应的文件夹名称。如果文件夹非空，则需要将文件夹下的所有object删除后才能删除改文件夹
